module.exports = function (RED) {
  ...
  function EnergyMeter(config) {
    ...
    function setValsToNode(oVals) {
      Object.keys(oVals).forEach((key) => {
        if (node.oValues[key] !== undefined) {
          if (key === "daily") {
            node.oValues.daily.value = Number(oVals.daily.value);
            node.oValues.daily.date = oVals.daily.date;
          } else {
            node.oValues[key] = Number(oVals[key]);
          }
        }
      });
    }
    ...
    node.on("input", async function (msg) {
      ...
      var today = new Date();
      var lastDate = node.oValues.daily.date
        ? new Date(node.oValues.daily.date)
        : null;

      // Check if we need to reset monthly (year or month changed)
      if (
        lastDate &&
      ...
      // Check if we need to reset weekly (week changed)
    ...
    function handleCommands(oPayload) {
      ...
      if (
        typeof oPayload === "string" &&
        COMMAND_TYPES.find((cmd) => cmd === oPayload)
      ) {
        switch (oPayload) {
          case "status":
          ...
          case "resetWeekly":
            node.oValues = {
              daily: {
                ...
                value: node.oValues.daily.value,
              },
              weekly: 0,
              monthly: node.oValues.monthly,
              yearly: node.oValues.yearly,
      ...
      if (
        typeof oPayload === "object" &&
        Object.keys(oPayload).find((cmd) => cmd === "set")
      ) {
      ...
      if (!oMessagePaylod) {
        ...
        oMessagePaylod = {
          error:
            "The input msg is not a supported command. Only the following are supported: " +
            sSupportedCommands,
        };
      }
